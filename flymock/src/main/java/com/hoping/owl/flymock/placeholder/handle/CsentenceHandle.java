 ...
 * @author houping wang
 */
public class CsentenceHandle extends AbstractPlaceholderHandle<String> {
    ...
    public String invoke(PlaceholderWrap placeholderWrap) {
        ...
        if(args.size() == 1) {
            return FlyRandom.csentence(Integer.parseInt(args.get(0)));
        }
        if(args.size() == 2) {
            return FlyRandom.csentence(Integer.parseInt(args.get(0)), Integer.parseInt(args.get(1)));
        }
        return FlyRandom.cparagraph();
    }
