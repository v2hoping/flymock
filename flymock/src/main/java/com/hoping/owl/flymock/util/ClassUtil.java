...
import com.hoping.owl.flymock.placeholder.handle.BooleanHandle;
import com.hoping.owl.flymock.placeholder.handle.NaturalHandle;
...
public class ClassUtil {
    ...
    public static TypeEnum getTypeEnum(Class clazz) {
    ...
    public static void main(String[] args) {
        // 包下面的类
        Set<Class<?>> clazzs = getClasses("com.alibaba.fastjson.util");
        if (clazzs == null) {
            return;
        }
        System.out.printf(clazzs.size() + "");
        // 某类或者接口的子类
        Set<Class<?>> clazzs1 = new HashSet<>();
        clazzs1.add(BooleanHandle.class);
        clazzs1.add(NaturalHandle.class);
        Set<Class<?>> inInterface = getByInterface(PlaceholderHandle.class, clazzs1);
    ...
    public static Set<Class<?>> getByInterface(Class clazz, String[] scanPackages) {
        Set<Class<?>> allClasses = new LinkedHashSet<>();
        for (String scanPackage : scanPackages) {
            Set<Class<?>> classes = ClassUtil.getClasses(scanPackage);
            allClasses.addAll(classes);
        }
        return ClassUtil.getByInterface(clazz, allClasses);
    }
