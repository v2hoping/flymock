 ...
 * @author houping wang
 */
public class TitleHandle extends AbstractPlaceholderHandle<String> {
    @Override
    public String invoke(PlaceholderWrap placeholderWrap) {
        List<String> args = placeholderWrap.getArgs();
        if(ArrayUtil.isEmpty(args)) {
            return FlyRandom.title();
        }
        if(args.size() == 1) {
            return FlyRandom.title(Integer.parseInt(args.get(0)));
        }
        if(args.size() == 2) {
            return FlyRandom.title(Integer.parseInt(args.get(0)), Integer.parseInt(args.get(1)));
        }
        return FlyRandom.title();
    }
