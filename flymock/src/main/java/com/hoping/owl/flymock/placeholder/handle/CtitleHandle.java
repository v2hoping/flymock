 ...
 * @author houping wang
 */
public class CtitleHandle extends AbstractPlaceholderHandle<String> {

    @Override
    public String invoke(PlaceholderWrap placeholderWrap) {
        List<String> args = placeholderWrap.getArgs();
        if(ArrayUtil.isEmpty(args)) {
            return FlyRandom.ctitle();
        }
        if(args.size() == 1) {
            return FlyRandom.ctitle(Integer.parseInt(args.get(0)));
        }
        if(args.size() == 2) {
            return FlyRandom.ctitle(Integer.parseInt(args.get(0)), Integer.parseInt(args.get(1)));
        }
        return FlyRandom.ctitle();
    }
